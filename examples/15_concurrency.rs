...
//
// Rust applique ses règles d'ownership et d'emprunt aussi entre
// threads : la plupart des erreurs classiques de concurrence (data
// race notamment) sont détectées à la compilation plutôt qu'à
// l'exécution.
//
...
fn main() {
    // thread::spawn démarre un thread. La closure doit être `move` dès
    // qu'elle utilise une donnée de l'extérieur, pour que le nouveau
    // thread en prenne clairement possession.
    let poignee = thread::spawn(|| {
        for i in 1..=3 {
            println!("thread enfant : étape {}", i);
            thread::sleep(Duration::from_millis(5));
        }
    });

    for i in 1..=2 {
        println!("thread principal : étape {}", i);
        thread::sleep(Duration::from_millis(5));
    }

    // join() bloque jusqu'à la fin du thread enfant : sans cet appel,
    // le programme pourrait se terminer avant que le thread ait fini.
    poignee.join().expect("le thread enfant a paniqué");

    // Arc<T> (Atomic Reference Counted) est l'équivalent thread-safe de
    // Rc<T> : plusieurs threads peuvent partager la possession d'une
    // même valeur. Mutex<T> garantit qu'un seul thread à la fois peut
    // accéder aux données qu'il protège (verrou posé/levé automatiquement
    // via le système de portée de Rust).
    ...
    for _ in 0..5 {
        ...
        let poignee = thread::spawn(move || {
            // lock() renvoie un Result (le verrou peut échouer si un
            // autre thread a paniqué en le détenant) : on utilise
            // unwrap() ici car c'est un cas d'école sans thread instable.
    ...
    println!(
        "compteur final après 5 threads : {}",
        *compteur_partage.lock().unwrap()
    );

    // Canaux (mpsc = multiple producer, single consumer) : une autre
    // manière de faire communiquer des threads, en s'envoyant des
    // messages plutôt qu'en partageant de la mémoire.
    ...
    drop(emetteur); // sans ce drop, le récepteur attendrait indéfiniment
