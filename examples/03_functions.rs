// 03 - Fonctions
//
// En Rust, chaque paramètre de fonction doit être typé, et le type de
// retour est annoncé avec `->`. La dernière expression d'un bloc (sans
// point-virgule) en devient la valeur de retour implicite.
//
// Pour lancer cet exemple :
//   cargo run --example 03_functions

// clippy::let_and_return désactivé volontairement : dans carre(), on
// garde le `let resultat = { ... }` intermédiaire pour bien séparer,
// à des fins pédagogiques, "le bloc est une expression" de "la fonction
// retourne sa dernière expression", plutôt que de fusionner les deux.
...
fn main() {
    ...
    // Une fonction sans valeur de retour renvoie le type unité `()`.
    afficher_ligne_de_titre("Fonctions");

    println!("carre(5) = {}", carre(5));
    println!("est_pair(10) = {}", est_pair(10));

    // Les fonctions sont des valeurs : on peut les passer en paramètre.
    println!("résultat via appliquer : {}", appliquer(carre, 6));

    match diviser(10, 0) {
        Some(resultat) => println!("10 / 0 = {}", resultat),
        None => println!("division par zéro refusée"),
    }
    match diviser(10, 2) {
...
// retournée. On aurait pu écrire `return left + right;` explicitement.
...
// expression grâce aux accolades.
fn carre(n: i32) -> i32 {
    let resultat = {
        let base = n;
...
// Une fonction peut recevoir une autre fonction en paramètre dès lors
// que sa signature (ici `fn(i32) -> i32`) correspond.
fn appliquer(f: fn(i32) -> i32, valeur: i32) -> i32 {
    f(valeur)
}

// Retourner un Option<T> est la manière idiomatique de signaler
// "peut-être pas de résultat" plutôt que de paniquer ou renvoyer -1.
